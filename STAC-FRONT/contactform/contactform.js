jQuery(document).ready(function($) {
  ...
  $('form.contactForm').submit(function() {
      ...
      if (rule !== undefined) {
        ...
        switch (rule) {
          ...
          case 'minlen':
            ...
            if (i.attr('name') == 'senha') i.on('input', checkPassword);
            break;
        }
        i.next('.validation').html((ierror ? (i.attr('data-msg') != undefined ? i.attr('data-msg') : 'wrong Input') : '')).show('blind');
      }
    ...
    let data = {};
    new FormData(document.querySelector("#contactForm")).forEach((value, key) => data[key] = value);
    ...
    .catch(result => { // Falha na requisição ou na obtenção dos valores de sucesso no getBody
      if (!result.isGetBody) {// Checa se é uma resposta da requisição ou da função getBody chamada no primeiro then
          ...
          showMessage('Não foi possível obter o status da requisição');
      }
    })
    .then(response => {
        let jsonObj = JSON.parse(response.value);
        if (response.ok) {
          $("#sendmessage").text(jsonObj.message).addClass("show");  
